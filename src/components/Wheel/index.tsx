...
export const Wheel = ({
        ...
        for (let i = 0; i < dataLength; i++) {
            ...
            wheelDataAux[i] = {
                ...
                style: {
                    ...
                    fontFamily: data[i].style?.fontFamily || fontFamily || DEFAULT_FONT_FAMILY,
                    fontSize: data[i].style?.fontSize || fontSize || DEFAULT_FONT_SIZE,
                    fontWeight: data[i].style?.fontWeight || fontWeight || DEFAULT_FONT_WEIGHT,
                    fontStyle: data[i].style?.fontStyle || fontStyle || DEFAULT_FONT_STYLE,
                    textColor:
    ...
    useEffect(() => {
        if (mustStartSpinning && !isCurrentlySpinning) {
            ...
            startSpinning();
            const selectedPrize =
                prizeMap[prizeNumber][Math.floor(Math.random() * prizeMap[prizeNumber]?.length)];
    ...
    return (
        <RouletteContainer
            style={
                !isFontLoaded || (totalImages > 0 && loadedImagesCounter !== totalImages)
                    ? { visibility: 'hidden' }
                    : {}
            }
        >
